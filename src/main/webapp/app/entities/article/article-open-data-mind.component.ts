...
@Component({
    selector: 'jhi-article-open-data-mind',
    templateUrl: './article-open-data-mind.component.html'
})
export class ArticleOpenDataMindComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/article-open-data-mind'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/article-open-data-mind', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: ArticleOpenDataMind) {
        return item.id;
    }
